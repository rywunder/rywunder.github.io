...
var server,
	...
	previous = 0, // keeps the previous value
	dr = false, // checks if it is doctor viewing
	arrhyth = [], //array of indexes of arrhythmias
	hr = [], //heartrate data
...
function read_annotations(t0_string) {
	...
	$.getJSON(url, function(data) {
		...
		//Creates an array of all arrhythmias indexes
		for(j = 0; j < ann[i].annotation.length; j++){
			var label = ann[i].annotation[j].a;
			var time = ann[i].annotation[j].t;
			if(label === 'A'){
				arrhyth.push(time);
			}
		}
...
//Checks current tick and alerts if there is an arrthymia.
function checkForArr(){


    var middle = t0_ticks + (dt_ticks/2);
    for (var index in arrhyth){
    	var target = arrhyth[index];
		if( (previous <= target) && (target <= middle) && (dr !== 'true')){
			$.post( "alert", {
				trigger: "arrhythmia",
				db: db,
				record: record,
				t0: t0_ticks/tickfreq,
				dt: dt_ticks/tickfreq				
			} );
		}
    }
    previous = middle;
...
function updateHeartrate(){
	...
	if(hr.length < middle){
		$('.scrollfwd').attr('disabled', 'disabled');
		$('.sfwd').attr('disabled', 'disabled');
		$('.eor').attr('disabled', 'disabled');
		$('.sfwd').attr('disabled', 'disabled');
		$('#heartrate').html("Retrieving heartrate data...");
		url = "data";
		url+= "?db=" + db;
		url+= "&record=" + record;
		$.getJSON(url, function(data) {
		if (data.success) { 
			hr = data.analysis.heartrate;
			$('#heartrate').html(Math.round(hr[middle]));
			
			$('.scrollfwd').removeAttr('disabled');
			$('.fwd').removeAttr('disabled');
			$('.eor').removeAttr('disabled');
...
function parse_url() {
    ...
    server = ('http://localhost:5000/lightwave');
	...
	else if (v[0] === 'dr') { dr = v[1]; } //Checks if doctor
    }
